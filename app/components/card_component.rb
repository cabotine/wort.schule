...
class CardComponent < ViewComponent::Base
  ...
  def initialize(word:, card:, default:)
    @word = word
    @card = card
    @default = default
  end
  ...
  def before_render
    @word_image_url = url_for @word.image
  end

  def liquid_template
    template = @default ? File.read(Rails.root.join("app/views/cards/default_noun.liquid")) : @card.template
    template_renderer = Liquid::Template.parse(template)
    @rendered_template = template_renderer.render(params.with_indifferent_access).html_safe
  end
